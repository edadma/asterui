...
|----------|------|-------------|
| `theme` | `string \| undefined` | The current theme. Only with ThemeProvider. |
...
| `toggleTheme` | `() => void \| undefined` | Toggle between light and dark theme. Only with ThemeProvider. |
| `systemTheme` | `'light' \| 'dark' \| undefined` | The system preference. Only with ThemeProvider. |

## getThemeColors()

For components that need hex color values (e.g., canvas, xterm), use the `getThemeColors()` utility:

```tsx
import { getThemeColors } from 'asterui'

// Call on demand — not reactive, call inside useEffect after theme changes
