...
The Tabs component supports the `data-testid` prop for e2e testing. Child elements receive suffixed test IDs:

```tsx
<Tabs data-testid="settings-tabs" items={items} />
```

| Element | Test ID |
|---------|---------|
| Container | `settings-tabs` |
| Tab list | `settings-tabs-tablist` |
| Tab panel | `settings-tabs-tabpanel` |
