...
The Mention component supports the `data-testid` prop for e2e testing. Child elements receive suffixed test IDs:

```tsx
<Mention data-testid="comment" options={users} />
```
...
| Dropdown | `comment-dropdown` |
| Loading state | `comment-loading` |
| Empty state | `comment-empty` |
