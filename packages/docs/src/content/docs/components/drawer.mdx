...
The Drawer component supports the `data-testid` prop for e2e testing. Child elements receive suffixed test IDs:

```tsx
<Drawer data-testid="settings" title="Settings" open={isOpen}>
  Content here
</Drawer>
```
