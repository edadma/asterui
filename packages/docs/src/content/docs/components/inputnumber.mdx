...
The InputNumber component supports the `data-testid` prop for e2e testing. Child elements receive suffixed test IDs:

```tsx
<InputNumber data-testid="quantity" />
```
...
| Increment button | `quantity-increment` |
| Decrement button | `quantity-decrement` |
