...
The VirtualList component supports the `data-testid` prop for e2e testing:

```tsx
<VirtualList data-testid="user-list" items={users} renderItem={renderUser} />
```

| Element | Test ID |
|---------|---------|
| Container | `user-list` |
| Inner scroll area | `user-list-inner` |
