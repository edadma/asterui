...
The Popconfirm component supports the `data-testid` prop for e2e testing. Child elements receive suffixed test IDs:

```tsx
<Popconfirm data-testid="delete" title="Delete?" onConfirm={handleDelete}>
...
| OK button | `delete-ok-button` |
| Cancel button | `delete-cancel-button` |
