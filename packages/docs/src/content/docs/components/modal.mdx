...
The Modal component supports the `data-testid` prop for e2e testing. Child elements receive suffixed test IDs:

```tsx
<Modal data-testid="confirm" title="Confirm" open={isOpen}>
  Are you sure?
</Modal>
```
...
| Close button | `confirm-close-button` |
| OK button | `confirm-ok-button` |
| Cancel button | `confirm-cancel-button` |
