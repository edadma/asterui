...
The MonthCalendar component supports the `data-testid` prop for e2e testing:

```tsx
<MonthCalendar data-testid="calendar" events={events} />
```
...
| Weekday header | `calendar-weekday-header` |
| Grid | `calendar-grid` |
