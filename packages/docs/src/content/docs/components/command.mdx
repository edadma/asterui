...
The Command component supports the `data-testid` prop for e2e testing. Child elements receive suffixed test IDs:

```tsx
<Command data-testid="cmd" open={isOpen} />
```
...
| Input | `cmd-input` |
| List | `cmd-list` |
| Empty state | `cmd-empty` |
| Footer | `cmd-footer` |
| Back button | `cmd-back` |
| Ungrouped items | `cmd-group-ungrouped` |
