...
| `toggleTheme` | `() => void` | Basculer entre le thème clair et sombre. |
| `colors` | `ThemeColors` | Couleurs du thème calculées en valeurs hex. Lazy — calculées uniquement lors de l'accès. |
...
- **Support Canvas** : Fournit des couleurs hex pour les contextes canvas/WebGL via la propriété `colors` (lazy — aucun coût sauf en cas d'accès)
