...
| `colors` | `ThemeColors` | Computed theme colors as hex values. Lazy — only computed when accessed. |
...
- **Canvas Support**: Provides hex colors for canvas/WebGL contexts via `colors` property (lazy — no cost unless accessed)
