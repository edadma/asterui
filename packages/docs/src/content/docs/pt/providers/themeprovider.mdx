...
| `toggleTheme` | `() => void` | Alternar entre tema claro e escuro. |
| `colors` | `ThemeColors` | Cores do tema calculadas como valores hex. Lazy — calculadas apenas quando acessadas. |
...
- **Suporte a Canvas**: Fornece cores hex para contextos canvas/WebGL através da propriedade `colors` (lazy — sem custo a menos que acessado)
